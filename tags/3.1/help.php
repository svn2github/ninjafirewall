...
function help_nfsubpolicies() {
	...
	get_current_screen()->add_help_tab( array(
		...
		<li>' . __('WordPress XML-RPC API:', 'ninjafirewall'). '<span class="description"> ' . __('XML-RPC is a remote procedure call (RPC) protocol which uses XML to encode its calls and HTTP as a transport mechanism. WordPress has an XMLRPC API that can be accessed through the <code>xmlrpc.php</code> file. Since WordPress version 3.5, it is always activated and cannot be turned off. NinjaFirewall allows you to immediately block any access to that file, or only to block an access using the <code>system.multicall</code> method (often used in brute-force amplification attacks). If you are using applications that need to access that API (e.g., JetPack plugin or the WordPress Androis/iOS app), enable only the <code>system.multicall</code> protection otherwise your application will be blocked.', 'ninjafirewall'). '</span></li>
